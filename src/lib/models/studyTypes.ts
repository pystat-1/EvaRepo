...
export async function createStudyType(
  ...
  if (!code) throw new Error("الرمز المختصر مطلوب");
  if (await prisma.studyType.findUnique({ where: { code } })) {
    throw new Error(`الرمز "${code}" مستخدم من قبل`);
  }
  const row = await prisma.studyType.create({
    data: { name: data.name, nameAr: data.nameAr ?? null, code },
  });
...
export async function updateStudyType(
  actorId: string,
  id: string,
  data: { name?: string; nameAr?: string; code?: string; active?: boolean }
): Promise<StudyType> {
  ...
  if (code && code !== before.code) {
    const clash = await prisma.studyType.findUnique({ where: { code } });
    if (clash && clash.id !== id) throw new Error(`الرمز "${code}" مستخدم من قبل`);
  }
